        ...
        <div className="flex items-start justify-between gap-2">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              {isAsk && <Badge variant="accent" size="sm">Ask</Badge>}
              {isShow && <Badge variant="accent" size="sm">Show</Badge>}
              {isJob && <Badge variant="secondary" size="sm">Job</Badge>}
            ...
            <div className="flex items-center gap-x-4 gap-y-2 text-xs text-gray-500 dark:text-gray-400 flex-wrap">
              <span className="flex items-center gap-1" title={`${story.score} points`}>
                <Icons.ArrowUp />
                {story.score}
              </span>

              <Link to={`/user/${story.by}`} className="flex items-center gap-1 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
                <Icons.User />
                {story.by}
              </Link>
