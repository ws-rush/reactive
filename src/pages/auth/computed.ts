...
type ComputedStateImpl = <T extends object, A extends object>(
  f: StateCreator<T & { computed?: (state: T) => A }, [], []>,
//   compute: (state: T) => A,
  opts?: ComputedStateOpts<T>,
) => StateCreator<T, [], [], T & A>
...
// function isComputeDefiened<T, A>(value: unknown): value is (state: T) => A {
//   return typeof value === "function"
// }

const computedImpl: ComputedStateImpl = (f, opts) => {
  // set of keys that have been accessed in any compute call
  const trackedSelectors = new Set<string | number | symbol>()

  return (set, get, api) => {
    const compute = f(set, get, api)?.computed
    if (!compute) {
...
export const extra = computedImpl as unknown as ComputedStateCreator
